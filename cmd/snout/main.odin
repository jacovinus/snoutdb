...
print_usage :: proc() {
	...
	fmt.eprintln("  snout log-info <file.log> [--format clf|combined|logfmt|syslog|regex] [--pattern \"...\"] [--strict]")
	...
	fmt.eprintln("              [--logformat clf|combined|logfmt|syslog|regex] [--logpattern \"...\"] [--strict]")
	fmt.eprintln("  snout -f <input> group=<column>[,<column>...] -- <aggregate>=<column|rows>... [options]")
	fmt.eprintln("    --where <column> <operator> [value]")
