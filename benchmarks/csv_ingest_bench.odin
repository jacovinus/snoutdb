...
main :: proc() {
	...
	for path in paths {
		...
		run_scanner_benchmark(path, info.size)
		run_inspection_benchmark(path)
		run_table_load_benchmark(path)
		fmt.println()
	}
}

run_scanner_benchmark :: proc(path: string, file_size: i64) {
	scanner, open_err := ingest.open_csv_scanner(path)
	if open_err != .None {
	...
	fmt.printfln(
		"scanner    %s: rows=%d fields=%d bytes=%d elapsed=%v MiB/s=%.1f rows/s=%.0f",
		path,
		rows,
		fields,
		file_size,
...
run_inspection_benchmark :: proc(path: string) {
	start := time.tick_now()
	schema, err := ingest.inspect_csv_file(path, "bench")
	elapsed := time.tick_since(start)
	if err != .None {
...
run_table_load_benchmark :: proc(path: string) {
	...
	table, table_err := ingest.populate_csv_table(path, &schema, context.allocator)
