...
SNIFF_JSONL_BENCH_FIXTURES :: [3]string {
...
main :: proc() {
	...
	if len(os.args) > 1 {
		...
		fixtures := SNIFF_JSONL_BENCH_FIXTURES
		paths = fixtures[:]
	}
	for path in paths {
		...
		run_direct_jsonl_sniff_benchmark(path, sniff.DEFAULT_SNIFF_CONFIG, "default")

		reduced := sniff.DEFAULT_SNIFF_CONFIG
		reduced.top_value_count = 0
		reduced.max_suggestions = 0
		run_direct_jsonl_sniff_benchmark(path, reduced, "reduced")
		fmt.println()
	}
}

run_direct_jsonl_sniff_benchmark :: proc(
	path: string,
	...
	report, err := sniff.profile_jsonl_file(path, "bench", config)
